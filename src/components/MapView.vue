  ...
  <div class="map-container">
          ...
          <div class="list-header">
            ...
            <div class="selection-info" v-if="hasSelection">
              <span class="start-marker">
                🟢 {{ state.start?.info?.displayName }}
              </span>
              <span class="end-marker" v-if="state.end">
                🔴 {{ state.end?.info?.displayName }}
              </span>
            </div>
          </div>
    ...
    >
      ...
      <span class="travel-text">
        🚋 Traveling from {{ state.start?.info?.displayName }}
        to {{ state.end?.info?.displayName }}
  ...
  setup () {
    ...
    const availableLines = computed(() => {
      ...
      state.features.forEach((f) => {
        const name = f.properties && f.properties.line
          ? String(f.properties.line)
          : 'unknown'
        if (name !== 'unknown') {
          set.add(name)
        }
    ...
    const filteredFeatures = computed(() => {
      ...
      if (searchQuery.value) {
        features = features.filter((feat) =>
          String(feat.properties.displayName || '')
            .toLowerCase()
      ...
      return features.sort((a, b) => {
        ...
        if (!aIsStation && bIsStation) return 1
        return (a.properties.displayName || '').localeCompare(
          b.properties.displayName || '',
        )
      })
    })
    ...
    */
    ...
    const styledVectorLayer = new VectorLayer({
      source: vectorSource,
      // keep "any" here to avoid TS conflicts with OpenLayers' FeatureLike
      style: (feature: any) => {
    ...
    function createDemoData (vecSource: VectorSource) {
      const stations: Record<string, number[]> = {
        ...
        suchsdorf: fromLonLat([10.115, 54.33]),
        rehbenitzwinkel: fromLonLat([10.125, 54.325]),
        wikkanal: fromLonLat([10.09, 54.355]),
        auberg: fromLonLat([10.1, 54.35]),
        knorrstrasse: fromLonLat([10.11, 54.345]),
        elendsredder: fromLonLat([10.12, 54.34]),
        hanssenstrasse: fromLonLat([10.13, 54.335]),
        belvedere: fromLonLat([10.14, 54.33]),
        schauspielhaus: fromLonLat([10.135, 54.32]),
        ansgarkirche: fromLonLat([10.145, 54.315]),
        schauenburgerstrasse: fromLonLat([10.155, 54.31]),
        dreiecksplatz: fromLonLat([10.145, 54.305]),
        lorentzendamm: fromLonLat([10.135, 54.3]),
        martensdamm: fromLonLat([10.125, 54.295]),
        kronshagen: fromLonLat([10.07, 54.335]),
        dehnckestrasse: fromLonLat([10.08, 54.33]),
        westring: fromLonLat([10.09, 54.325]),
        wilhelmsplatz: fromLonLat([10.1, 54.32]),
        eserzieplatz: fromLonLat([10.11, 54.315]),
        ziegelreich: fromLonLat([10.12, 54.31]),
        hauptbahnhof: fromLonLat([10.132, 54.315]),
        projensdorf: fromLonLat([10.16, 54.35]),
        torfende: fromLonLat([10.15, 54.345]),
        bremerskamp: fromLonLat([10.14, 54.34]),
        leibnizstrasse: fromLonLat([10.13, 54.335]),
        unisportstatten: fromLonLat([10.125, 54.33]),
        uniipn: fromLonLat([10.12, 54.325]),
        uniaudimax: fromLonLat([10.115, 54.32]),
        samwerstrasse: fromLonLat([10.11, 54.315]),
      }
      ...
      demoLines.forEach((lineCfg) => {
        ...
        for (let i = 0; i < keys.length - 1; i++) {
          ...
          if (a && b && stationGraph[a] && stationGraph[b]) {
            if (!stationGraph[a].neighbors.includes(b)) {
              stationGraph[a].neighbors.push(b)
            }
            if (!stationGraph[b].neighbors.includes(a)) {
              stationGraph[b].neighbors.push(a)
            }
          }
        }
      })
      ...
      if (!startKey || !endKey || !stationGraph[startKey] || !stationGraph[endKey]) {
        ...
        const lineFeatures: Feature<LineString>[] = features.filter(
          (f): f is Feature<LineString> =>
            !!f.getGeometry && f.getGeometry()?.getType() === 'LineString',
        )
        if (!lineFeatures.length) return

        const startCoord =
          state.start?.coord ?? state.end?.coord ?? fromLonLat([10.133, 54.3233])
